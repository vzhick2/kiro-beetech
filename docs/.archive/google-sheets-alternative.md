...
_Purpose: Creates the custom menu in Google Sheets_
...
function createNewPurchase() {
  const sheet =
    SpreadsheetApp.getActiveSpreadsheet().getSheetByName('Purchases');
...
function editSelectedPurchase() {
  ...
  if (activeRow === 1) {
    SpreadsheetApp.getUi().alert(
      'Please select a purchase row, not the header.'
    );
    return;
  }
...
function recalculateAllWAC() {
  ...
  if (result === SpreadsheetApp.getUi().Button.YES) {
    try {
      ...
      SpreadsheetApp.getUi().alert(
        'Error during WAC recalculation: ' + e.message
      );
    }
  }
}
...
_Purpose: Core purchase management logic_
...
function getPurchaseDetails(purchaseId) {
  ...
  const itemsData = itemsSheet
    .getRange(2, 1, itemsSheet.getLastRow() - 1, 3)
    .getValues();
...
_Purpose: Production batch management_
...
_Purpose: Utility functions for inventory management_
...
function updateInventory(itemId, quantityChange, newCost, transactionType) {
  const itemsSheet =
    SpreadsheetApp.getActiveSpreadsheet().getSheetByName('Items');
  ...
  if (transactionType === 'PURCHASE') {
    ...
    if (newStock > 0) {
      const newTotalValue =
        currentStock * currentWac + quantityChange * newCost;
...
function recalculateWACForAllItems() {
  ...
  const itemsData = itemsSheet
    .getRange(2, 1, itemsSheet.getLastRow() - 1, 8)
    .getValues();
  const transData = transSheet
    .getRange(2, 1, transSheet.getLastRow() - 1, 10)
    .getValues();
...
_Purpose: Purchase editing interface_
...
<html>
  ...
  <body>
    ...
    <button class="finalize-btn" onclick="finalize()" id="finalizeBtn">
      Finalize Purchase
    </button>
    ...
    <script>
      ...
      function addLineItem(existingItem = null) {
        ...
        itemDiv.innerHTML = `
          ...
          <input type="number" class="cost-input" placeholder="Unit Cost" value="${existingItem ? existingItem.cost : 0.0}" step="0.01" min="0">
