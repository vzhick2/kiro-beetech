...
-- Add columns to track cost breakdown in purchase_line_items  
...
BEGIN
  ...
  SELECT COALESCE(SUM(pli.quantity * pli.base_unit_cost), 0)
  INTO v_inventory_subtotal
  FROM purchase_line_items pli
  JOIN items i ON pli.itemid = i.itemid
  WHERE pli.purchaseid = p_purchase_id
    AND NOT pli.is_overhead_item
  ...
  SELECT 
    pli.itemid,
    pli.base_unit_cost,
    (pli.base_unit_cost * v_allocation_ratio)::NUMERIC(10,4) as allocated_overhead,
    (pli.base_unit_cost * (1 + v_allocation_ratio))::NUMERIC(10,4) as final_unit_cost
  FROM purchase_line_items pli
  JOIN items i ON pli.itemid = i.itemid  
  WHERE pli.purchaseid = p_purchase_id
    AND NOT pli.is_overhead_item
...
BEGIN
  ...
  LOOP
    -- Update purchase line item with allocation breakdown
    UPDATE purchase_line_items 
    ...
    PERFORM update_item_wac(
      ...
      (SELECT quantity FROM purchase_line_items WHERE purchaseid = p_purchase_id AND itemid = v_allocation.item_id),
  ...
  FROM purchases p
  LEFT JOIN purchase_line_items pli ON p.purchaseid = pli.purchaseid
  WHERE p.purchaseid = p_purchase_id
