...
import { getSuppliers, updateSupplier, deleteSupplier, bulkDeleteSuppliers, bulkArchiveSuppliers } from '@/app/actions/suppliers'
...
export const suppliersKeys = {
  ...
  list: (filters: { searchQuery?: string }) => [...suppliersKeys.lists(), filters] as const,
}
...
export function useSuppliers(searchQuery = '') {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const transformedSuppliers: Supplier[] = (result.data || []).map((dbSupplier: any) => ({
...
export function useUpdateSupplier() {
  ...
  return useMutation({
    mutationFn: async ({ supplierId, updates }: { supplierId: string; updates: Record<string, unknown> }) => {
