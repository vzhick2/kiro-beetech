...
const generateMockData = (): Supplier[] => {
  ...
  return Array.from({ length: 150 }, (_, i) => {
    ...
    return {
      ...
      notes: hasNotes
        ? `Reliable supplier since ${2015 + yearOffset}. ${notes[noteIndex]!}`
        : '',
      status: statuses[statusIndex]! as 'active' | 'inactive',
      createdAt: new Date(
        new Date('2024-01-01').getTime() + daysAgo * 24 * 60 * 60 * 1000
      ),
    };
  }) as Supplier[];
};
...
export const useDataTable = () => {
  ...
  const filteredData = useMemo(() => {
    ...
    if (globalFilter) {
      filtered = filtered.filter(item =>
        matchesSearchQuery(item, globalFilter)
      );
    }
  ...
  const updateMultipleSuppliers = async (
    updates: Array<{ id: string; data: Partial<Supplier> }>
  ) => {
    setLoading(true);
    try {
  ...
  const unarchiveSuppliers = async (ids: string[]) => {
    ...
    try {
      ...
      setData(prev =>
        prev.map(item =>
          ids.includes(item.id) ? { ...item, status: 'active' as const } : item
        )
      );
  ...
  const exportSuppliers = async (ids: string[], format: 'csv' | 'excel') => {
    ...
    try {
      ...
      console.log(
        `Exporting ${suppliersToExport.length} suppliers as ${format}`
      );
