...
import { Check, X, Edit3, Trash2, Plus, MoreHorizontal, Minus } from 'lucide-react'
import { Item, InventoryUnit } from '@/types'
import { Button } from '@/components/ui/button'
import { DropdownMenu, DropdownMenuContent, DropdownMenuItem, DropdownMenuTrigger } from '@/components/ui/dropdown-menu'
import { useItems, useUpdateItem, useBulkDeleteItems, useBulkArchiveItems } from '@/hooks/use-items'
...
export function SpreadsheetTable({ searchQuery = '', typeFilter = 'all' }: SpreadsheetTableProps) {
  ...
  const handleCellEdit = useCallback((itemId: string, field: keyof Item, value: string | number | boolean | Date) => {
    setEditingCell({ itemId, field, value, isEditing: true })
  }, [])

  const handleCellSave = useCallback(async (itemId: string, field: keyof Item, value: string | number | boolean | Date) => {
    try {
      ...
      const dbValue = field === 'lastCountedDate' && value && typeof value !== 'boolean' ? new Date(value).toISOString() : value
      
      // Use React Query mutation
      await updateItemMutation.mutateAsync({ itemId, updates: { [dbField]: dbValue } })
      setEditingCell(null)
    } catch (error) {
  ...
  const renderCell = useCallback((item: Item, field: keyof Item) => {
    const value = item[field]
    const isEditing = editingCell?.itemId === item.itemId && editingCell?.field === field
        ...
        return (
          <div className="flex items-center space-x-1">
            ...
            <input
              ...
              onKeyDown={(e) => {
                if (e.key === 'Enter') {
                  handleCellSave(item.itemId, field, Number(e.currentTarget.value))
                } else if (e.key === 'Escape') {
                  handleCellCancel()
                }
              }}
              onBlur={(e) => handleCellSave(item.itemId, field, Number(e.target.value))}
              autoFocus
            />
            <button
              onClick={() => {
    ...
    if (field === 'lastCountedDate' && value && typeof value === 'object' && 'toLocaleDateString' in value) {
    ...
    if ((field === 'created_at' || field === 'updated_at') && value && typeof value === 'object' && 'toLocaleDateString' in value) {
      return <span>{(value as Date).toLocaleDateString()}</span>
    }
          ...
          <table className="w-full min-w-[600px] md:min-w-[900px]">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-200">
                ...
                <th className="text-left p-3 font-semibold text-gray-700 min-w-[100px]">Type</th>
                <th className="text-left p-3 font-semibold text-gray-700 min-w-[140px]">Quantity</th>
                <th className="text-left p-3 font-semibold text-gray-700 min-w-[100px]">Unit Cost</th>
                <th className="text-left p-3 font-semibold text-gray-700 min-w-[120px]">Reorder Point</th>
                <th className="text-left p-3 font-semibold text-gray-700 min-w-[150px]">Last Used Supplier</th>
                <th className="text-left p-3 font-semibold text-gray-700 min-w-[100px]">Last Counted</th>
                <th className="text-left p-3 font-semibold text-gray-700 min-w-[80px]">Status</th>
                <th className="w-12 p-3">
              ...
              {filteredItems.map((item) => (
                <tr 
                  ...
                  className={`border-b border-gray-100 hover:bg-gray-50 transition-all duration-200 ${
                    selectedRows.has(item.itemId) ? 'bg-blue-50 ring-1 ring-blue-200' : ''
                  }`}
                >
                  <td className="p-3">
                    <div className="flex items-center justify-center">
                  ...
                  <td className="p-3">{renderCell(item, 'weightedAverageCost')}</td>
                  <td className="p-3">{renderCell(item, 'reorderPoint')}</td>
                  <td className="p-3">
                    <div className="text-sm">
                      {item.lastUsedSupplier ? (
                        <span className="text-gray-900">{item.lastUsedSupplier}</span>
                      ) : (
                        <span className="text-gray-400 italic">No purchases yet</span>
                      )}
                    </div>
                  </td>
                  ...
                  <td className="p-3">
                    <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${
                      item.isArchived ? 'bg-gray-100 text-gray-800' : 'bg-green-100 text-green-800'
                    }`}>
                  ...
                  <td className="p-3">
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end" className="w-48">
                        <DropdownMenuItem onClick={() => handleCellEdit(item.itemId, 'name', item.name)}>
                          <Edit3 className="w-4 h-4 mr-2" />
                          Edit
                        </DropdownMenuItem>
                        <DropdownMenuItem onClick={() => handleQuickReorder(item.itemId)}>
                          <Plus className="w-4 h-4 mr-2" />
                          Quick Reorder
                        </DropdownMenuItem>
                        <DropdownMenuItem onClick={() => handleManualCount(item.itemId)}>
                          <Check className="w-4 h-4 mr-2" />
                          Manual Count
                        </DropdownMenuItem>
                        <DropdownMenuItem 
                          onClick={() => updateItemMutation.mutate({ itemId: item.itemId, updates: { isarchived: true } })}
        ...
        <div className="flex items-center justify-between text-sm text-gray-700 flex-wrap gap-4">
          <div className="flex items-center space-x-6">
            ...
            <span className="text-orange-600 font-semibold">
              {filteredItems.filter(i => i.currentQuantity <= (i.reorderPoint || 0)).length} items below reorder point
            </span>
          </div>
          <div className="flex items-center space-x-2">
            <span className="font-semibold">Total value:</span>
            <span className="text-green-600 font-bold text-lg">
              ${filteredItems.reduce((sum, item) => sum + (item.currentQuantity * item.weightedAverageCost), 0).toFixed(2)}
            </span>
          </div>
        </div>
      </div>
