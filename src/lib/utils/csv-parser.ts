...
export function parseQBOSalesCSV(csvContent: string): CSVParsingResult {
  ...
  for (let i = 0; i < dataRows.length; i++) {
    ...
    try {
      ...
      // Validate row
      const validatedRow = QBOSalesRowSchema.parse(
        rowData as Record<string, string>
      );

      // Skip non-sales transactions
      if (
        validatedRow['Transaction Type'] !== 'Sale' &&
        validatedRow['Transaction Type'] !== 'Invoice'
      ) {
        continue;
      }
      ...
      const itemName =
        validatedRow['Product/Service'] || validatedRow['Description'] || '';
 ...
 */
export function validateQBOFormat(csvContent: string): {
  isValid: boolean;
  errors: string[];
} {
  const lines = csvContent.split('\n').filter(line => line.trim());
