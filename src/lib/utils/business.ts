...
import { CycleCountAlert, Item, PurchaseLineItem, Recipe /* RecipeIngredient */ } from '@/types'
...
export function calculateCycleCountPriority(item: Item): number {
  const daysSinceCount = item.lastCountedDate 
    ? Math.floor((Date.now() - item.lastCountedDate.getTime()) / (1000 * 60 * 60 * 24))
    : 365 // Default to 1 year if never counted

  const countScore = daysSinceCount / 30
  const stockScore = 1 - (item.currentQuantity / Math.max(item.reorderPoint || 1, 1))
 ...
 */
export function generateCycleCountAlerts(items: Item[], limit: number = 5): CycleCountAlert[] {
 ...
 */
export function calculateCostVariance(actualCost: number, projectedCost: number): number {
 ...
 */
export function calculateReorderQuantity(item: Item, averageUsage?: number): number {
...
export function validatePurchaseTotals(
  ...
  const lineItemsTotal = lineItems.reduce((sum, item) => sum + item.totalCost, 0)
...
export function allocateAdditionalCosts(
  ...
  const inventoryTotal = inventoryItems.reduce((sum, item) => sum + item.totalCost, 0)
  ...
  return lineItems.map(lineItem => {
    const isInventoryItem = inventoryItems.some(inv => inv.lineItemId === lineItem.lineItemId)
